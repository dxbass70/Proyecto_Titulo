...
public class MazeGenerator : MonoBehaviour {
    ...
    [Tooltip("edificio prefab object.")]
    [SerializeField]
    private GameObject edifPrefab;
    ...
    public void EdificioSalida(CellScript cScript, int wallID)
    {
        if (wallID == 1){
            SpriteRenderer spriterender = cScript.wallL.GetComponent<SpriteRenderer>();
            SpriteRenderer spriterenderedif = edifPrefab.GetComponent<SpriteRenderer>();
            spriterender.sprite = spriterenderedif.sprite;
            cScript.wallL.transform.position = cScript.wallL.transform.position + new Vector3(-1,0,0);
            cScript.wallL.transform.rotation = Quaternion.Euler(new Vector3(0,0,0));
            BoxCollider2D collider = cScript.wallL.GetComponent<BoxCollider2D>();
            cScript.wallL.tag = "Objetivo";
            collider.size = new Vector2(2, 1);
            collider.isTrigger = true;
        }
        else if (wallID == 2){
            SpriteRenderer spriterender = cScript.wallR.GetComponent<SpriteRenderer>();
            SpriteRenderer spriterenderedif = edifPrefab.GetComponent<SpriteRenderer>();
            spriterender.sprite = spriterenderedif.sprite;
            cScript.wallR.transform.position = cScript.wallR.transform.position + new Vector3(1,0,0);
            cScript.wallR.transform.rotation = Quaternion.Euler(new Vector3(0,0,0));
            BoxCollider2D collider = cScript.wallR.GetComponent<BoxCollider2D>();
            cScript.wallR.tag = "Objetivo";
            collider.size = new Vector2(2, 1);
            collider.isTrigger = true;
        }
        else if (wallID == 3){
            SpriteRenderer spriterender = cScript.wallU.GetComponent<SpriteRenderer>();
            SpriteRenderer spriterenderedif = edifPrefab.GetComponent<SpriteRenderer>();
            spriterender.sprite = spriterenderedif.sprite;
            cScript.wallU.transform.position = cScript.wallU.transform.position + new Vector3(0,1,0);
            cScript.wallU.transform.rotation = Quaternion.Euler(new Vector3(0,0,0));
            BoxCollider2D collider = cScript.wallU.GetComponent<BoxCollider2D>();
            cScript.wallU.tag = "Objetivo";
            collider.size = new Vector2(1, 2);
            collider.isTrigger = true;
            
        }
        else if (wallID == 4){
            SpriteRenderer spriterender = cScript.wallD.GetComponent<SpriteRenderer>();
            SpriteRenderer spriterenderedif = edifPrefab.GetComponent<SpriteRenderer>();
            spriterender.sprite = spriterenderedif.sprite;
            cScript.wallD.transform.position = cScript.wallD.transform.position + new Vector3(0,-1,0);
            cScript.wallD.transform.rotation = Quaternion.Euler(new Vector3(0,0,0));
            BoxCollider2D collider = cScript.wallD.GetComponent<BoxCollider2D>();
            cScript.wallD.tag = "Objetivo";
            collider.size = new Vector2(1, 2);
            collider.isTrigger = true;
        }
    }
